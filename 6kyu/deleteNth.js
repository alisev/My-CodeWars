...
// Luckily, Alice and Bob are able to encode the motif as a number. Can you help them to remove numbers such that their list contains each number only up to N times, without changing the order?
...
function deleteNth(arr,n){
  ...
  return arr.filter((el) => {
    seen[el] = seen.hasOwnProperty(el) ? seen[el] + 1 : 1;
    return seen[el] <= n;
  });
}
...
function deleteNth(arr,n){
  let seen = {};
  return arr.filter(el => (seen[el] = seen[el] + 1 || 1) <= n);
}
